...
const CANVAS_OPS = CanvasOp[
    ...
    CanvasOp(:measure_text_buf_font_ascent, [], F,
        "() => { const m = ctx.measureText(S.buf); return m.fontBoundingBoxAscent ?? m.actualBoundingBoxAscent ?? 0; }"),
    CanvasOp(:measure_text_buf_font_descent, [], F,
        "() => { const m = ctx.measureText(S.buf); return m.fontBoundingBoxDescent ?? m.actualBoundingBoxDescent ?? 0; }"),
